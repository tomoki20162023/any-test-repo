...
- リモートリポジトリのブランチでfast-forwardマージされるようにしておく必要がある
	- そうでないとpushが拒否される
...
- リモートリポジトリとローカルリポジトリの変更履歴の差分によって、取り込む方法が異なる
	- 単純にリモートリポジトリの方が変更履歴が進んでいる場合は、fast-forwardマージが実行される
	- どちらのリポジトリも変更されていた場合
		- 競合がない場合はマージコミット
		- 競合がある場合は、修正してからコミットする必要がある

## フェッチ(fetch)

- pullを実行すると、基本的にリモートの変更履歴がマージされる
- 変更履歴は取り込みたくないが、変更内容は確認したいときにフェッチを実行する
- 名前のないブランチとして取り込まれ、FETCH_HEADとしてチェックアウトできる
...
- リリースタグとしてバージョンを管理し、コメントや署名を付けておくのに使われることが多い
